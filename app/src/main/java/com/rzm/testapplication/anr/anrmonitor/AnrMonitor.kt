...
class AnrMonitor(lifecycle: Lifecycle) : LifecycleObserver {
    ...
    private val mThreadRunnable = Runnable {
        ...
        if (!mainHandlerRunEnd && !isDebugger()) {
            LogUtils.log("mThreadRunnable: main thread may be block at least $blockTime s")
        }
        ...
        if (blockTime >= ARN_TIMEOUT_SECOND) {
            if (!mainHandlerRunEnd && !isDebugger() && !mHadReport) {
                mHadReport = true
                //5s了，主线程还没更新这个标志，ANR
                LogUtils.log("ANR->main thread may be block at least $blockTime s ")
                LogUtils.log(getMainThreadStack())
