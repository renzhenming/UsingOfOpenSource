...
public class AppStartup {
    ...
    private final List<OnTaskListener> projectListeners;
    ...
    final Config config;
    @NonNull
    final LogInfo logger;

    private AppStartup(Builder builder) {
    ...
    private void notifyStageFinish() {
        for (OnTaskListener onProjectListener : projectListeners) {
            onProjectListener.onStageFinish();
        }
    }

    public AppStartup start() {
    ...
    private void onProjectStart() {
        if (projectListeners != null && !projectListeners.isEmpty()) {
            for (OnTaskListener listener : projectListeners) {
    ...
    public void addOnProjectExecuteListener(OnTaskListener listener) {
        projectListeners.add(listener);
    }

    public static class Builder {

        private final List<OnTaskListener> projectListeners = new ArrayList<OnTaskListener>();
        ...
        private LogInfo logger;

        public AppStartup create() {
            if (config == null) {
                config = new Config();
            }
            if (logger == null) {
                logger = new LogInfo.DefaultLogger();
            }
            return new AppStartup(this);
        }

        public Builder addOnProjectExecuteListener(OnTaskListener listener) {
            projectListeners.add(listener);
            return Builder.this;
        }
        ...
        public Builder setLogger(LogInfo logger) {
            this.logger = logger;
            return Builder.this;
        }
